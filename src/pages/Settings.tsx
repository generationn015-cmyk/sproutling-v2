...
export function SettingsPage() {
  const { user, saveUser, enableReminders, disableReminders, pairing, createPairing, exportData, deleteAll, signOut } = useApp();
  ...
  return (
    <div className="max-w-xl">
      ...
      <section className="mt-8 rounded-[24px] bg-parchment p-5">
        ...
        <p className="serif mt-1 text-2xl text-forest">{user.displayName || "You"}</p>
        <p className="mt-1 text-sm text-mute">{user.email}</p>
        <p className="mt-3 text-xs text-mute">
          Signed in on this device. Add to Home Screen in Safari for the full iPhone app chrome. Updates apply the next time you open the installed app.
        </p>
        <Button className="mt-4" tone="line" onClick={() => setConfirmOut(true)}>
          Sign out
        </Button>
      </section>
      ...
      <section className="mt-4 rounded-[24px] bg-parchment p-5">
        ...
        <Field value={user.displayName} placeholder="Used in the Today greeting" onChange={(e) => void saveUser({ displayName: e.target.value })} />
      ...
      <section className="mt-4 rounded-[24px] bg-parchment p-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <Switch label="Gentle reminders" checked={user.reminderEnabled} onChange={(on) => void (on ? enableReminders() : disableReminders())} />
        </div>
        <div className="mt-4">
          <Label>Reminder hour</Label>
          <input type="range" min={6} max={21} value={user.reminderHour} onChange={(e) => void saveUser({ reminderHour: Number(e.target.value) })} className="w-full" />
      ...
      <section className="mt-4 rounded-[24px] bg-parchment p-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <Switch label="Quiet motion" checked={user.reducedMotion} onChange={(on) => void saveUser({ reducedMotion: on })} />
        </div>
      </section>
      ...
      <section className="mt-4 rounded-[24px] bg-parchment p-5">
        ...
        <div className="flex gap-2">
          {(["imperial", "metric"] as const).map((u) => (
            <button key={u} className={`rounded-full px-4 py-2 text-sm ${user.units === u ? "bg-forest text-parchment-2" : "bg-parchment-2"}`} onClick={() => void saveUser({ units: u })}>
              {u}
            </button>
          ))}
        </div>
      ...
      <section className="mt-4 rounded-[24px] bg-parchment p-5">
        ...
        <p className="mt-1 text-sm text-mute">Short-lived 6-digit code. It expires in 15 minutes and is not a standing secret for the whole collection.</p>
        {pairing && !expired ? (
      ...
      <Sheet open={confirmOut} title="Sign out?" onClose={() => setConfirmOut(false)}>
        <p className="text-sm text-mute">Your plants stay on this device. You will need your email and password to come back.</p>
        <div className="mt-4 flex gap-3">
          <Button tone="clay" onClick={() => { signOut(); setConfirmOut(false); }}>
            Sign out
          </Button>
          <Button tone="line" onClick={() => setConfirmOut(false)}>
            Stay signed in
          </Button>
        </div>
      </Sheet>
      ...
      <Sheet open={confirmWipe} title="Delete everything?" onClose={() => setConfirmWipe(false)}>
        ...
        <div className="mt-4 flex gap-3">
          <Button tone="clay" onClick={async () => { await deleteAll(); setConfirmWipe(false); }}>
            Delete all data
          </Button>
